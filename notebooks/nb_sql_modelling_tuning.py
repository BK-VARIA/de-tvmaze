...
display(
    spark.sql(
       ...
       ROUND(AVG(e.runtime), 1)               AS avg_runtime,
       ROUND(AVG(s.rating),  2)               AS avg_show_rating
FROM  silver.silver_show_genres g
LEFT JOIN silver.silver_shows s ON g.show_id = s.show_id
LEFT JOIN silver.silver_episodes e ON e.show_id = s.show_id
GROUP BY g.genre
HAVING COUNT(DISTINCT s.show_id)"""
    )
...
# MAGIC %md
# MAGIC ### Put Constraints on the table so data quality is met before ingestion of data 

# COMMAND ----------

# %sql
# ALTER TABLE silver.silver_episodes
#   ALTER COLUMN episode_id SET NOT NULL;
# ALTER TABLE silver.silver_episodes
#   ADD CONSTRAINT runtime_positive CHECK (runtime IS NULL OR runtime > 0);
# ALTER TABLE silver.silver_shows
